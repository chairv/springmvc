...
public class UDPClientA {
    public static void main(String[] args) {
        try {
            // 向server发起请求
            SocketAddress target = new InetSocketAddress("103.214.143.85", 2008);
    ...
    private static void receive(DatagramSocket client) {
        try {
            for (;;) {
                ...
                UDPServer.send(reportMessage, port, address, client);
            }
        } catch (Exception e) {
            e.printStackTrace();
        }
...
//    //回复内容
//    public static void sendMessaage(String reportMessage, int port, InetAddress address, DatagramSocket client) {
//        try {
